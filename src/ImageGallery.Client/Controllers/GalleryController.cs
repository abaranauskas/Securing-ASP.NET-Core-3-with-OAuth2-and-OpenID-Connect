...
namespace ImageGallery.Client.Controllers
{
    ...
    public class GalleryController : Controller
    {
        ...
        {
            await HttpContext.SignOutAsync(CookieAuthenticationDefaults.AuthenticationScheme); //logs out from client deletes cokie from browser
            await HttpContext.SignOutAsync(OpenIdConnectDefaults.AuthenticationScheme); // logs out from identity provider

            //var idpClient = _httpClientFactory.CreateClient("IDPClient");

            //var metaDataResponse = await idpClient.GetDiscoveryDocumentAsync(); //this extension from identityModel nuget
        ...
        //[Authorize(Roles = "PayingUser, abc, andSoOn")]
        ...
        public async Task<IActionResult> OrderFrame()
        {
            ...
            {
                throw new Exception("Problem accessing the discovery endpoint", userInfoResponse.Exception);
            }

            var address = userInfoResponse.Claims.FirstOrDefault(x => x.Type == JwtClaimTypes.Address)?.Value;
